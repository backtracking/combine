...
\begin{center}
  ...
  \end{array} \right)$}
\qquad\qquad\qquad
  \includegraphics[scale=0.5]{dlx.mps}
\end{center}
Les pointeurs s'étendent jusqu'aux \noeuds\ suivants,
circulairement. Les entêtes sont représentés en rouge.
...
ci-dessus, par soucis de clarté). Le champ \of{s} contient le nom de
...
correspondante et est utilisé pour choisir une colonne minimisant ce
nombre ; pour tout autre \noeud, il contient le numéro de la ligne de
...
mémoire, auxquels s'ajoutent son entrée dans la table de
...
mémoïzation et elle de fait des plus efficace.
Le code effectuant la mémoïzation est factorisé dans des opérateurs de
point fixes effectuant. Ainsi le cardinal d'un ZDD s'écrit aussi
simplement que
\begin{ocaml}
...
\paragraph{\of{Tiling}.}
Il n'y a pas grand à dire concernant le module \of{Tiling}, si ce
...
Toujours dans ce module, les tables de mémoïsation des différentes
...
déjà à l'\oe uvre en OCaml dans l'outil Why3~\cite{why3manual072} et
